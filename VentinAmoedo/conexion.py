...
#=================================================== SERVICIOS ===================================================
    ...
    def altaServicio(newservicio):
        try:
            query = QtSql.QSqlQuery()
            queryCli = QtSql.QSqlQuery()
            query.prepare('insert into servicios (codigo, concepto, precio-unidad) '
                          'values (:codigo, :concepto, :precio-unidad)')
    ...
    def modServicio(self=None):
        try:
            codigo = var.ui.tabProductos.selectedItems()[0].data(0)
    ...
    def oneServicio(codigo):
        try:
            registro = []
            query = QtSql.QSqlQuery()
            query.prepare('select codigo, concepto, precio-unidad from servicios where codigo = :codigo')
